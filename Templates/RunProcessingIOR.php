...
// Copy the entire 'IOR' directory from templates to working directory
$IOR_template_dir = $temp_dir . "Anybody\\IOR";
$IOR_work_dir = $work_dir . "\\IOR";
recurse_copy($IOR_template_dir, $IOR_work_dir);

// Copy specific files for Subject SQ1
$IOR_subjects_sq1_dir = $temp_dir . "Anybody\\IOR\\Subjects\\SQ1";
$IOR_subjects_sq1_work_dir = $work_dir . "\\IOR\\Subjects\\SQ1";
@mkdir($IOR_subjects_sq1_work_dir, 0777, true);
$specific_files = ['SubjectSpecificData.any']; // The specific file(s) to copy

foreach ($specific_files as $specific_file) {
    if (file_exists($IOR_subjects_sq1_dir . "\\" . $specific_file)) {
        copy($IOR_subjects_sq1_dir . "\\" . $specific_file, $IOR_subjects_sq1_work_dir . "\\" . $specific_file);
    }
}
...
foreach ($params_xml->group as $group) {
    if ((string)$group['name'] == 'measurement_guids') {
        foreach ($group->param as $param) {
            ...
            if (preg_match('/Static 1|IOR FB \d+/', $trial_name)) {
                // Static 1 will be copied directly, IOR FB trials will be copied from Dynamic 1
...
$c3d_files_dir = $work_dir . "\\IOR\\c3d_files";
...
foreach ($dynamic_trials as $trial_name) {
    ...
    // Determine if it's a IOR FB trial and not the first one
    if (strpos($trial_name, 'IOR FB') !== false && $trial_name != 'IOR FB 1') {
        $source_dir = 'IOR FB 1'; // Use IOR FB 1 as the source for all other IOR FB trials
    }

    $source_path = $IOR_subjects_sq1_dir . "\\" . $source_dir;
    $dest_path = $IOR_subjects_sq1_work_dir . "\\" . $trial_name;
...
$subject_specific_data_file = $xml_directory . "\\IOR\\Subjects\\SQ1\\SubjectSpecificData.any";
