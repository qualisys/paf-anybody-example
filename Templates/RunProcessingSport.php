...
// Copy the entire 'Sport' directory from templates to working directory
$sport_template_dir = $temp_dir . "Anybody\\Sport";
$sport_work_dir = $work_dir . "\\Sport";
recurse_copy($sport_template_dir, $sport_work_dir);

// Copy specific files for Subject SQ1
$sport_subjects_sq1_dir = $temp_dir . "Anybody\\Sport\\Subjects\\SQ1";
$sport_subjects_sq1_work_dir = $work_dir . "\\Sport\\Subjects\\SQ1";
@mkdir($sport_subjects_sq1_work_dir, 0777, true);
$specific_files = ['SubjectSpecificData.any']; // The specific file(s) to copy

foreach ($specific_files as $specific_file) {
    if (file_exists($sport_subjects_sq1_dir . "\\" . $specific_file)) {
        copy($sport_subjects_sq1_dir . "\\" . $specific_file, $sport_subjects_sq1_work_dir . "\\" . $specific_file);
    }
}
...
foreach ($params_xml->group as $group) {
    if ((string)$group['name'] == 'measurement_guids') {
        foreach ($group->param as $param) {
            ...
            if (preg_match('/Static 1|Sport FB \d+/', $trial_name)) {
                // Static 1 will be copied directly, Sport FB trials will be copied from Dynamic 1
...
$c3d_files_dir = $work_dir . "\\Sport\\c3d_files";
...
foreach ($dynamic_trials as $trial_name) {
    ...
    // Determine if it's a Sport FB trial and not the first one
    if (strpos($trial_name, 'Sport FB') !== false && $trial_name != 'Sport FB 1') {
        $source_dir = 'Sport FB 1'; // Use Sport FB 1 as the source for all other Sport FB trials
    }

    $source_path = $sport_subjects_sq1_dir . "\\" . $source_dir;
    $dest_path = $sport_subjects_sq1_work_dir . "\\" . $trial_name;
...
$subject_specific_data_file = $xml_directory . "\\Sport\\Subjects\\SQ1\\SubjectSpecificData.any";
